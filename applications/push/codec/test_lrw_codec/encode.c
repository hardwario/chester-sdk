...
ZTEST(app_push_lrw, test_positive)
{
	...
	g_app_data.meteo.wind_speed.last_sample = 21.25f;
	g_app_data.meteo.wind_direction.last_sample = 192;
	g_app_data.meteo.rainfall.last_sample = 10;

	CTR_TEST_LRW_JSON_METEO(data);
#endif
...
// 1-Wire
	...
	g_app_data.barometer.pressure.last_sample = 96000;

	CTR_TEST_LRW_JSON_BAROMETER_TAG(data)
#endif

#if defined(FEATURE_HARDWARE_CHESTER_Z)
	g_app_data.backup.line_voltage = 48;
	g_app_data.backup.battery_voltage = 12;
	g_app_data.backup.line_present = 1;

	CTR_TEST_LRW_JSON_BACKUP(data);
	CTR_TEST_LRW_JSON_PUSH(data);
#endif
...
ZTEST(app_push_lrw, test_negative)
{
	cJSON *root = cJSON_CreateObject();
	cJSON *data = cJSON_CreateObject();
	cJSON_AddItemToObject(root, "data", data);
...
// 1-Wire
	...
	g_app_data.button[0].event_count = 1;
	g_app_data.button[1].event_count = 1;
	g_app_data.button[2].event_count = 1;
	g_app_data.button[3].event_count = 1;
	g_app_data.button[4].event_count = 1;

	g_app_data.button[0].click_count = 4;
	g_app_data.button[1].click_count = 5;
	g_app_data.button[2].click_count = 6;
	g_app_data.button[3].click_count = 7;
	g_app_data.button[4].click_count = 8;

	g_app_data.button[0].hold_count = 7;
	g_app_data.button[1].hold_count = 8;
	g_app_data.button[2].hold_count = 9;
	g_app_data.button[3].hold_count = 10;
	g_app_data.button[4].hold_count = 11;

	g_app_data.button[0].events[0].type = APP_DATA_BUTTON_EVENT_X_HOLD;
	g_app_data.button[1].events[0].type = APP_DATA_BUTTON_EVENT_X_HOLD;
	g_app_data.button[2].events[0].type = APP_DATA_BUTTON_EVENT_X_HOLD;
	g_app_data.button[3].events[0].type = APP_DATA_BUTTON_EVENT_X_HOLD;
	g_app_data.button[4].events[0].type = APP_DATA_BUTTON_EVENT_X_HOLD;
...
#if defined(FEATURE_HARDWARE_CHESTER_METEO_A) || defined(FEATURE_HARDWARE_CHESTER_METEO_B)
	g_app_data.meteo.wind_speed.last_sample = NAN;
	g_app_data.meteo.wind_direction.last_sample = NAN;
	g_app_data.meteo.rainfall.last_sample = NAN;
	CTR_TEST_LRW_JSON_METEO(data);
#endif
...
// 1-Wire
	...
	g_app_data.backup.line_present = 0;
...
ZTEST_SUITE(app_push_lrw, NULL, NULL, NULL, NULL, NULL);
