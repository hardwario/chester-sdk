...
static int encode(zcbor_state_t *zs)
{
	...
	if (g_app_config.channel_a1_active || g_app_config.channel_a2_active ||
		...
		zcbor_uint32_put(zs, CODEC_KEY_E_WEIGHT);
		{
			zcbor_list_start_encode(zs, ZCBOR_VALUE_IS_INDEFINITE_LENGTH);

			zcbor_int64_put(zs, g_app_data.weight_measurement_timestamp);
	...
	/* ^^^ Preserved code "weight" (end) */

	/* ### Preserved code "people_counter" (begin) */

#if defined(FEATURE_CHESTER_HARDWARE_PEOPLE_COUNTER)
	zcbor_uint32_put(zs, CODEC_KEY_E_PEOPLE_COUNTER);
	{
		zcbor_list_start_encode(zs, ZCBOR_VALUE_IS_INDEFINITE_LENGTH);
...
#endif /* defined(FEATURE_CHESTER_HARDWARE_PEOPLE_COUNTER) */

	/* ^^^ Preserved code "people_counter" (end) */
