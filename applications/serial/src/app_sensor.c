...
static void aggreg_sample(float *samples, size_t count, struct ctr_data_aggreg *sample)
{
	...
	for (int i = 0; i < CTR_BLE_TAG_COUNT; i++) {
		...
		if (g_app_data.ble_tag.sensor[i].sample_count < APP_DATA_MAX_BLE_TAG_SAMPLES) {
			...
			if (valid) {
				...
				LOG_INF("BLE tag %d: sample count: %d", i, sensor->sample_count);
			} else {
				sensor->last_sample_temperature = NAN;
				sensor->last_sample_humidity = NAN;
				sensor->rssi = 0;
				sensor->voltage = NAN;
				LOG_INF("BLE tag %d: no valid data", i);
			}

			app_data_unlock();
		} else {
			LOG_WRN("BLE tag sample buffer full");
			return -ENOSPC;
		}
	}
...
int app_sensor_ble_tag_aggreg(void)
{
	...
	for (int i = 0; i < CTR_BLE_TAG_COUNT; i++) {
		...
		if (ble_tag->sensor[i].measurement_count < APP_DATA_MAX_BLE_TAG_MEASUREMENTS) {
			...
			LOG_WRN("BLE tag measurement buffer full");
			app_data_unlock();
			return -ENOSPC;
		}
