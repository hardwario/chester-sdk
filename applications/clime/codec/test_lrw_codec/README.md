...
In `encode.c` it is important to keep different encode blocks in specific order, because cJSON serializes JSON as it is called one by another, however Node decodes that in a different way. So to compare them, objects must have specific place.
