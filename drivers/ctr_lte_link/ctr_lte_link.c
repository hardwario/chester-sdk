...
static inline const struct ctr_lte_link_config *get_config(const struct device *dev)
{
	return dev->config;
}
...
static void receive_character(const struct device *dev, char c, bool *received_line)
{
	...
	if (!atomic_get(&data->rx_line_synced)) {
		...
		if (c == '\n') {
			...
			LOG_WRN("Not synced character: %c (0x%02x)", c, c);
		}
...
static void receive_in_data_mode(const struct device *dev)
{
	...
	if (data->user_cb && !atomic_get(&data->in_dialog)) {
		data->user_cb(data->dev, CTR_LTE_LINK_EVENT_RX_DATA, data->user_data);
	}
}
...
static void rx_receive_work_handler(struct k_work *work)
{
	struct ctr_lte_link_data *data =
		CONTAINER_OF(work, struct ctr_lte_link_data, rx_receive_work);
...
static void uart_callback(const struct device *dev, struct uart_event *evt, void *user_data)
{
	...
	struct ctr_lte_link_data *data = get_data((const struct device *)user_data);
...
static int ctr_lte_link_drv_init(const struct device *dev)
{
	...
	const struct ctr_lte_link_config *config = get_config(dev);
	struct ctr_lte_link_data *data = get_data(dev);
...
#define CTR_LTE_LINK_INIT(n)                                                                       \
	static const struct ctr_lte_link_config inst_##n##_config = {                              \
	...
	DEVICE_DT_INST_DEFINE(n, ctr_lte_link_drv_init, NULL, &inst_##n##_data,                    \
			      &inst_##n##_config, POST_KERNEL, CONFIG_SERIAL_INIT_PRIORITY,        \
			      &ctr_lte_link_driver_api);
