...
#define DT_DRV_COMPAT hardwario_ctr_lrw_link

LOG_MODULE_REGISTER(ctr_lrw_link, CONFIG_CTR_LRW_LINK_LOG_LEVEL);
...
struct ctr_lrw_link_config {
	const struct device *uart_dev;
	const struct gpio_dt_spec reset_spec;
};

static inline const struct ctr_lrw_link_config *get_config(const struct device *dev)
{
	return dev->config;
}

static inline struct ctr_lrw_link_data *get_data(const struct device *dev)
{
	return dev->data;
}
...
static void process_line(const struct device *dev)
{
	...
	struct ctr_lrw_link_data *data = get_data(dev);
	...
	if (!p) {
		...
		if (data->user_cb) {
			data->user_cb(data->dev, CTR_LRW_LINK_EVENT_RX_LOSS, data->user_data);
		}
	...
	if (data->user_cb && !atomic_get(&data->in_dialog)) {
		data->user_cb(data->dev, CTR_LRW_LINK_EVENT_RX_LINE, data->user_data);
	}
}

static void receive_character(const struct device *dev, char c, bool *received_line)
{
	if (received_line) {
		*received_line = false;
	}
	...
	if (!atomic_get(&data->rx_line_synced)) {
		data->rx_line_len = 0;

		if (c == '\n') {
			atomic_set(&data->rx_line_synced, true);
		} else {
			LOG_WRN("Unsynced character: %c (0x%02x)", c, c);
		}
	...
	if (c == '\n') {
		...
		if (atomic_get(&data->recv_line)) {
			atomic_set(&data->recv_line, false);
		} else {
	...
	if (data->rx_line_len >= sizeof(data->rx_line_buf) - 1) {
		LOG_ERR("Line buffer overflow");
...
static void rx_receive_work_handler(struct k_work *work)
{
	struct ctr_lrw_link_data *data =
		CONTAINER_OF(work, struct ctr_lrw_link_data, rx_receive_work);

	if (atomic_get(&data->in_dialog)) {
		return;
	}
...
static void rx_restart_work_handler(struct k_work *work)
{
	...
	ret = uart_rx_enable(get_config(data->dev)->uart_dev, buf, RX_SLAB_BLOCK_SIZE, RX_TIMEOUT);
	if (ret) {
...
static void uart_callback(const struct device *dev, struct uart_event *evt, void *user_data)
{
	...
	struct ctr_lrw_link_data *data = get_data((const struct device *)user_data);
...
static int ctr_lrw_link_set_callback_(const struct device *dev, ctr_lrw_link_user_cb user_cb,
				      void *user_data)
{
	if (k_is_in_isr()) {
		return -EWOULDBLOCK;
...
static void ctr_lrw_link_lock_(const struct device *dev)
{
	k_mutex_lock(&get_data(dev)->lock, K_FOREVER);
}

static void ctr_lrw_link_unlock_(const struct device *dev)
{
	k_mutex_unlock(&get_data(dev)->lock);
}

static int ctr_lrw_link_reset_(const struct device *dev)
{
	int ret;
...
static int ctr_lrw_link_enable_uart_(const struct device *dev)
{
	int ret;
	...
	ret = uart_rx_enable(get_config(dev)->uart_dev, buf, RX_SLAB_BLOCK_SIZE, RX_TIMEOUT);
	if (ret) {
		LOG_ERR("Call `uart_rx_enable` failed: %d", ret);
...
static int ctr_lrw_link_disable_uart_(const struct device *dev)
{
	int ret;
...
static int ctr_lrw_link_enter_dialog_(const struct device *dev)
{
	if (k_is_in_isr()) {
		return -EWOULDBLOCK;
	}
...
static int ctr_lrw_link_exit_dialog_(const struct device *dev)
{
	int ret;
...
static int ctr_lrw_link_send_line_(const struct device *dev, k_timeout_t timeout,
...
static int ctr_lrw_link_recv_line_(const struct device *dev, k_timeout_t timeout, char **line)
{
	int ret;
	...
	if (get_data(dev)->in_dialog) {
		...
		while (!received_line) {
			...
			receive_character(dev, c, &received_line);
		}
	}
	...
	*line = k_fifo_get(&get_data(dev)->rx_fifo, K_NO_WAIT);
	if (*line) {
		...
		atomic_set(&get_data(dev)->recv_line, true);
	}
...
static int ctr_lrw_link_free_line_(const struct device *dev, char *line)
{
	if (k_is_in_isr()) {
		return -EWOULDBLOCK;
	}
...
static int ctr_lrw_link_drv_init(const struct device *dev)
{
	int ret;

	const struct ctr_lrw_link_config *config = get_config(dev);
	struct ctr_lrw_link_data *data = get_data(dev);
...
static const struct ctr_lrw_link_driver_api ctr_lrw_link_driver_api = {
	.set_callback = ctr_lrw_link_set_callback_,
	.lock = ctr_lrw_link_lock_,
	.unlock = ctr_lrw_link_unlock_,
	.reset = ctr_lrw_link_reset_,
	.enable_uart = ctr_lrw_link_enable_uart_,
	.disable_uart = ctr_lrw_link_disable_uart_,
	.enter_dialog = ctr_lrw_link_enter_dialog_,
	.exit_dialog = ctr_lrw_link_exit_dialog_,
	.send_line = ctr_lrw_link_send_line_,
	.recv_line = ctr_lrw_link_recv_line_,
	.free_line = ctr_lrw_link_free_line_,
};

#define CTR_LRW_LINK_INIT(n)                                                                       \
	static const struct ctr_lrw_link_config inst_##n##_config = {                              \
		.uart_dev = DEVICE_DT_GET(DT_INST_BUS(n)),                                         \
		.reset_spec = GPIO_DT_SPEC_INST_GET(n, reset_gpios),                               \
	};                                                                                         \
	static struct ctr_lrw_link_data inst_##n##_data = {                                        \
		.dev = DEVICE_DT_INST_GET(n),                                                      \
	};                                                                                         \
	DEVICE_DT_INST_DEFINE(n, ctr_lrw_link_drv_init, NULL, &inst_##n##_data,                    \
			      &inst_##n##_config, POST_KERNEL, CONFIG_SERIAL_INIT_PRIORITY,        \
			      &ctr_lrw_link_driver_api);

DT_INST_FOREACH_STATUS_OKAY(CTR_LRW_LINK_INIT)
