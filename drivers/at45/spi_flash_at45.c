...
LOG_MODULE_REGISTER(ctr_spi_flash_at45, CONFIG_FLASH_LOG_LEVEL);
...
{
	struct spi_flash_at45_data *dev_data = dev->data;
	const struct spi_flash_at45_config *dev_config = dev->config;

	k_work_cancel_delayable(&dev_data->work);
...
static void release(const struct device *dev)
{
	...
	k_work_reschedule(&dev_data->work, K_MSEC(CONFIG_SPI_FLASH_AT45_SLEEP_TIMEOUT));
}
...
static void sleep_work_handler(struct k_work *work)
{
	...
	struct spi_flash_at45_data *dev_data =
		CONTAINER_OF(work_delayable, struct spi_flash_at45_data, work);
...
static int spi_flash_at45_init(const struct device *dev)
{
	...
	k_work_init_delayable(&dev_data->work, sleep_work_handler);
	dev_data->is_sleeping = false;
...
#define INST_RESET_GPIO_SPEC(idx)                                                                  \
	IF_ENABLED(INST_HAS_RESET_GPIO(idx), (static const struct gpio_dt_spec reset_##idx =       \
...
#define SPI_FLASH_AT45_INST(idx)                                                                   \
	...
	static const struct spi_flash_at45_config inst_##idx##_config = {                          \
		.bus = SPI_DT_SPEC_INST_GET(idx, SPI_OP_MODE_MASTER | SPI_TRANSFER_MSB |           \
							 SPI_WORD_SET(8)),                         \
		IF_ENABLED(INST_HAS_RESET_GPIO(idx), (.reset = &reset_##idx, ))                    \
		...
		.sector_0a_size =                                                                  \
			DT_INST_PROP(idx, sector_0a_pages) * DT_INST_PROP(idx, page_size),         \
