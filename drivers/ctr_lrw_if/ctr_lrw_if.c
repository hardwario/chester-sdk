...
struct ctr_lrw_if_config {
	const struct device *uart_dev;
	const struct gpio_dt_spec reset_spec;
};

struct ctr_lrw_if_data {
...
static inline const struct ctr_lrw_if_config *get_config(const struct device *dev)
{
	return dev->config;
}

static inline struct ctr_lrw_if_data *get_data(const struct device *dev)
{
	return dev->data;
}
...
static void rx_receive_work_handler(struct k_work *work)
{
	...
	struct ctr_lrw_if_data *data = CONTAINER_OF(work, struct ctr_lrw_if_data, rx_receive_work);
...
static void rx_restart_work_handler(struct k_work *work)
{
	...
	struct ctr_lrw_if_data *data = CONTAINER_OF(work, struct ctr_lrw_if_data, rx_restart_work);
...
static void uart_callback(const struct device *dev, struct uart_event *evt, void *user_data)
{
	...
	const struct device *ctr_lrw_if_dev = (const struct device *)user_data;

	switch (evt->type) {
	case UART_TX_DONE:
		LOG_DBG("Event `UART_TX_DONE`");
		k_poll_signal_raise(&get_data(ctr_lrw_if_dev)->tx_done_sig, 0);
		break;
	...
	case UART_RX_RDY:
		...
		if (evt->data.rx.len > 0) {
			...
			if (ring_buf_put(&get_data(ctr_lrw_if_dev)->rx_ring_buf,
					 rx->buf + rx->offset, rx->len) != rx->len) {
				atomic_set(&get_data(ctr_lrw_if_dev)->reset, true);
			}

			ret = k_work_submit(&get_data(ctr_lrw_if_dev)->rx_receive_work);
	...
	case UART_RX_BUF_REQUEST:
		...
		ret = k_mem_slab_alloc(&get_data(ctr_lrw_if_dev)->rx_slab, (void **)&buf,
				       K_NO_WAIT);
	...
	case UART_RX_DISABLED:
		...
		if (atomic_get(&get_data(ctr_lrw_if_dev)->stop)) {
			k_poll_signal_raise(&get_data(ctr_lrw_if_dev)->rx_disabled_sig, 0);

		} else {
			ret = k_work_submit(&get_data(ctr_lrw_if_dev)->rx_restart_work);
...
static int ctr_lrw_if_init_(const struct device *dev, ctr_lrw_recv_cb recv_cb)
{
	int ret;
...
static int ctr_lrw_if_reset_(const struct device *dev)
{
	int ret;
...
static int ctr_lrw_if_enable_(const struct device *dev)
{
	int ret;
...
static int ctr_lrw_if_disable_(const struct device *dev)
{
	int ret;
...
static int ctr_lrw_if_send_(const struct device *dev, const char *fmt, va_list ap)
{
	int ret;
...
static int ctr_lrw_if_drv_init(const struct device *dev)
{
	int ret;
...
static const struct ctr_lrw_if_driver_api ctr_lrw_if_driver_api = {
	.init = ctr_lrw_if_init_,
	.reset = ctr_lrw_if_reset_,
	.enable = ctr_lrw_if_enable_,
	.disable = ctr_lrw_if_disable_,
	.send = ctr_lrw_if_send_,
};

#define CTR_LRW_IF_INIT(n)                                                                         \
	static const struct ctr_lrw_if_config inst_##n##_config = {                                \
		.uart_dev = DEVICE_DT_GET(DT_INST_BUS(n)),                                         \
		.reset_spec = GPIO_DT_SPEC_INST_GET(n, reset_gpios),                               \
	};                                                                                         \
	static struct ctr_lrw_if_data inst_##n##_data = {                                          \
		.dev = DEVICE_DT_INST_GET(n),                                                      \
	};                                                                                         \
	DEVICE_DT_INST_DEFINE(n, ctr_lrw_if_drv_init, NULL, &inst_##n##_data, &inst_##n##_config,  \
			      POST_KERNEL, CONFIG_SERIAL_INIT_PRIORITY, &ctr_lrw_if_driver_api);

DT_INST_FOREACH_STATUS_OKAY(CTR_LRW_IF_INIT)
