...
#define ASTRONODE_S_CMD_SSC_W 		0x07 	/* Satellite Search Configuration Write Request. Stored in RAM (never saved in NVM). */
...
#define ASTRONODE_S_CMD_PLD_E 		0x25 	/* Enqueue uplink payload, and store in RAM (never saved in NVM) */
#define ASTRONODE_S_CMD_PLD_D 		0x26 	/* Dequeue uplink payload */
#define ASTRONODE_S_CMD_PLD_F 		0x27 	/* Clear (Free) all queued payloads */
#define ASTRONODE_S_CMD_GEO_W 		0x35 	/* Write geolocation longitude and latitude, and store in RAM. */
#define ASTRONODE_S_CMD_SAK_R 		0x45 	/* Read Acknowledgement */
#define ASTRONODE_S_CMD_SAK_C 		0x46 	/* Confirm to the module that the Acknowledgement was properly decoded and can be deleted by the module */
#define ASTRONODE_S_CMD_CMD_R 		0x47 	/* Read a command (downlink) */
#define ASTRONODE_S_CMD_CMD_C 		0x48 	/* Confirm to the module that the command was properly decoded and can be deleted by the module */
...
#define ASTRONODE_S_CMD_END_R 		0x6B 	/* Environment Details Read Request to evaluate RF environment */
#define ASTRONODE_S_CMD_HTX_S 		0x6C 	/* Homologation Transmit Start Request (special firmware only) */

#define ASTRONODE_S_ANSWER_CODE(command_id) ((command_id) | 0x80)
#define ASTRONODE_S_ANSWER_ERROR	0xFF

#define ASTRONODE_S_ERROR_CRC_NOT_VALID			0x0001 /* Discrepancy between provided CRC and expected CRC.*/
#define ASTRONODE_S_ERROR_LENGTH_NOT_VALID		0x0011 /* Message length is not valid for the operation code, or the message exceeds the maximum length for a frame.*/
...
#define ASTRONODE_S_ERROR_FORMAT_NOT_VALID		0x0601 /* At least one of the fields (SSID, password, token) is not composed of exclusively printable standard ASCII characters (0x20 to 0x7E).*/
#define ASTRONODE_S_ERROR_PERIOD_INVALID		0x0701 /* The Satellite Search Config period enumeration value is not valid*/
#define ASTRONODE_S_ERROR_BUFFER_FULL			0x2501 /* Failed to queue the payload because the sending queue is already full.*/
#define ASTRONODE_S_ERROR_DUPLICATE_ID			0x2511 /* Failed to queue the payload because the Payload ID provided by the asset is already in use in the module queue.*/
#define ASTRONODE_S_ERROR_BUFFER_EMPTY			0x2601 /* Failed to dequeue a payload from the buffer because the buffer is empty*/
#define ASTRONODE_S_ERROR_INVALID_POS			0x3501 /* Failed to update the geolocation information. Latitude and longitude fields must in the range [-90,90] degrees and [-180,180] degrees, respectively.*/
#define ASTRONODE_S_ERROR_NO_ACK				0x4501 /* No satellite acknowledgement available for any payload.*/
#define ASTRONODE_S_ERROR_NO_ACK_CLEAR			0x4601 /* No payload ack to clear, or it was already cleared.*/
#define ASTRONODE_S_ERROR_NO_COMMAND			0x4701 /* No command is available.*/
#define ASTRONODE_S_ERROR_NO_COMMAND_CLEAR		0x4801 /* No command to clear, or it was already cleared.*/
#define ASTRONODE_S_ERROR_MAX_TX_REACHED		0x6101 /* Failed to test Tx due to the maximum number of transmissions being reached.*/
