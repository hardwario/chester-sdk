...
enum ctr_lte_event {
	...
	CTR_LTE_EVENT_SEND_OK = 6,
	CTR_LTE_EVENT_SEND_ERR = 7,
};
...
int ctr_lte_set_event_cb(ctr_lte_event_cb user_cb, void *user_data);
...
typedef struct {
	enum {
		CTR_LTE_EVENT_ATTACH_DONE = 0,
		CTR_LTE_EVENT_ATTACH_ERROR = 1,
		CTR_LTE_EVENT_DETACH_DONE = 2,
		CTR_LTE_EVENT_DETACH_ERROR = 3,
		CTR_LTE_EVENT_SEND_DONE = 4,
		CTR_LTE_EVENT_SEND_ERROR = 5,
		CTR_LTE_EVENT_RECV_DONE = 6,
	} source;
	union {
		struct {
			const void *buf;
...
typedef void (*ctr_lte_callback_t)(ctr_lte_event_t *event, void *param);
