...
void main(void)
{
	...
	ret = people_counter_set_power_off_delay(dev, 23);
	if (ret) {
		LOG_ERR("Call `people_counter_set_power_off_delay` failed: %d", ret);
		k_oops();
	}

	ret = people_counter_set_stay_timeout(dev, 27);
	if (ret) {
		LOG_ERR("Call `people_counter_set_stay_timeout` failed: %d", ret);
		k_oops();
	}

	ret = people_counter_set_adult_border(dev, 29);
	if (ret) {
	...
	for (;;) {
		...
		if (ret) {
			LOG_ERR("Call `people_counter_read_data` failed: %d", ret);
			k_oops();
		}
		...
		LOG_INF("Parameter [Power off delay]: %d", value);
		...
		LOG_INF("Parameter [Stay timeout]: %d", value);
		...
		LOG_INF("Parameter [Adult border]: %d", value);
