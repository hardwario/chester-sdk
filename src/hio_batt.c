...
static int measure(uint16_t *voltage)
{
    ...
    if (result < 0) {
        ...
        *voltage = (uint32_t)(result >> 4) * 2048 *
                   (DIVIDER_R1 + DIVIDER_R2) / (2047 * DIVIDER_R2);
    }
...
static int cmd_batt_measure(const struct shell *shell,
                            size_t argc, char **argv)
{
    ...
    shell_print(shell, "voltage (rest): %u millivolts",
                result.voltage_rest_mv);
    shell_print(shell, "voltage (load): %u millivolts",
                result.voltage_load_mv);
    shell_print(shell, "current (load): %u milliamps",
                result.current_load_ma);

    return 0;
}

static int cmd_batt_load(const struct shell *shell,
                         size_t argc, char **argv)
{
    int ret;
...
static int cmd_batt_unload(const struct shell *shell,
                         size_t argc, char **argv)
{
    int ret;
