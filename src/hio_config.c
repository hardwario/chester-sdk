...
static int cmd_reset(const struct shell *shell, size_t argc, char **argv)
{
    ...
    ret = flash_area_get_sectors(FLASH_AREA_ID(storage), &sector_count,
                                 &flash_sector);
    ...
    uint16_t nvs_sector_size = CONFIG_SETTINGS_NVS_SECTOR_SIZE_MULT *
...
SHELL_STATIC_SUBCMD_SET_CREATE(
    ...
    SHELL_CMD_ARG(save, NULL, "Save all configuration.",
                  cmd_save, 1, 0),
    SHELL_CMD_ARG(reset, NULL, "Reset all configuration.",
                  cmd_reset, 1, 0),
