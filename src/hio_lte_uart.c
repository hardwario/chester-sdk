...
static void process_urc(const char *buf, size_t len)
{
...
int hio_lte_uart_init(void)
{
	...
	hio_sys_task_init(&rx_task, "hio_lte_uart_rx", rx_task_stack,
	                  HIO_SYS_TASK_STACK_SIZEOF(rx_task_stack), rx_task_entry, NULL);
...
int hio_lte_uart_disable(void)
{
	...
	struct k_poll_event events[] = { K_POLL_EVENT_INITIALIZER(
		K_POLL_TYPE_SIGNAL, K_POLL_MODE_NOTIFY_ONLY, &rx_disabled_sig) };
