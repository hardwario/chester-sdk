...
int hio_chester_z_get_status(struct hio_chester_z_status *status)
{
    ...
    status->dc_input_connected =
        (reg_status & 0x0040) != 0 ? true : false;
    status->button_0_pressed =
        (reg_status & 0x0010) != 0 ? true : false;
    status->button_1_pressed =
        (reg_status & 0x0008) != 0 ? true : false;
    status->button_2_pressed =
        (reg_status & 0x0004) != 0 ? true : false;
    status->button_3_pressed =
        (reg_status & 0x0002) != 0 ? true : false;
    status->button_4_pressed =
        (reg_status & 0x0001) != 0 ? true : false;
                          ...
                          const struct hio_chester_z_led_param *param)
{
    ...
    reg_led = (uint16_t)param->brightness << 8 |
              (uint16_t)param->command << 4 | (uint16_t)param->pattern;
